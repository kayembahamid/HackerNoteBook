...
{% embed url="https://youtu.be/5WfiTHiU4x8?si=3SzWOHw7hcblSAIw" %}
...
{% embed url="https://youtu.be/Bvx-hDHkIBk?si=JribsjwOQX9dzFOd" %}
...
{% embed url="https://youtu.be/mJ_5qeqGOaI?si=12Z70fHOjv51PwsB" %}
...
{% embed url="https://youtu.be/B1vqKQIPxr0?si=60WsCyDtuGOF-kua" %}
...
{% embed url="https://youtu.be/OD2vG5st4zI?si=-OWXRqRiwlj450Gw" %}
...
* [https://en.wikipedia.org/wiki/List\\\\\_of\\\\\_IP\\\\\_protocol\\\\\_numbers](https://en.wikipedia.org/wiki/List/_of/_IP/_protocol/_numbers)
...
| ------------------------------------------------------ | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RIP (Routing Information Protocol)**                 | A distance-vector protocol that uses hop count as its metric.            | <p>- <strong>RIPv1</strong>: Broadcasts routing tables every 30 seconds.<br>- <strong>RIPv2</strong>: Supports multicast updates sent to multicast <code>224.0.0.9</code>, and CIDR notation.<br>- AD = 120</p> |
| **OSPF (Open Shortest Path First)**                    | A link-state protocol that uses bandwidth to determine the best path.    | <p>- Protocol number: 89<br>- Uses Multicast addresses:<br><code>224.0.0.5</code> (normal communication),<br><code>224.0.0.6</code> (updates to designated routers).<br>- AD = 110</p>                          |
| **EIGRP (Enhanced Interior Gateway Routing Protocol)** | A hybrid protocol combining distance-vector and link-state features.     | <p>- AD = 5 (summary routes), 90 (internal), 170 (external).<br>- Uses metrics like bandwidth, delay, and reliability</p>                                                                                       |
| **BGP (Border Gateway Protocol)**                      | A path-vector protocol used for routing between autonomous systems (AS). | <p>- <strong>eBGP</strong>: AD = 20 (external).<br>- <strong>iBGP</strong>: AD = 200 (internal).</p>                                                                                                            |
...
| **Protocol**                            | **Description**                                                     | **Key Features**                                                                                                                                               |
| --------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TCP (Transmission Control Protocol)** | A connection-oriented protocol that ensures reliable data delivery. | <p>- Provides error checking, retransmission, and flow control.<br>- Used for applications like web browsing (HTTP/HTTPS) and email (SMTP, IMAP).</p>          |
| **UDP (User Datagram Protocol)**        | A connectionless protocol that prioritizes speed over reliability.  | <p>- No error checking or retransmission, making it faster but less reliable.<br>- Used for real-time applications like video streaming and online gaming.</p> |
...
Complete List of known ports: [https://en.wikipedia.org/wiki/List\\\\\_of\\\\\_TCP\\\\\_and\\\\\_UDP\\\\\_port\\\\\_numbers](https://en.wikipedia.org/wiki/List/_of/_TCP/_and/_UDP/_port/_numbers)
...
<table data-header-hidden><thead><tr><th></th><th></th><th></th></tr></thead><tbody><tr><td><strong>Scan Type</strong></td><td><strong>Description</strong></td><td><strong>Example Command</strong></td></tr><tr><td><strong>Basic SYN Scan</strong></td><td>Performs a stealthy TCP SYN scan to detect open ports.</td><td><pre><code>nmap -sS $target
</code></pre></td></tr><tr><td><strong>UDP Scan</strong></td><td>Scans for open UDP ports.</td><td><pre><code>nmap -sU $target
</code></pre></td></tr><tr><td><strong>Version Detection</strong></td><td>Detects the version of services running on open ports.</td><td><pre><code>nmap -sV $target
</code></pre></td></tr><tr><td><strong>OS Detection</strong></td><td>Identifies the operating system of the target.</td><td><pre><code>nmap -O $target
</code></pre></td></tr><tr><td><strong>Aggressive Scan</strong></td><td>Combines OS detection, version detection, script scanning, and traceroute.</td><td><pre><code>nmap -A $target
</code></pre></td></tr><tr><td><strong>Scan Specific Ports</strong></td><td>Scans specific ports on the target.</td><td><pre><code>nmap -p 22,80,443 $target
</code></pre></td></tr><tr><td><strong>Scan Entire Subnet</strong></td><td>Scans all devices in a subnet.</td><td><pre><code>nmap 192.168.1.0/24
</code></pre></td></tr><tr><td><strong>Ping Scan</strong></td><td>Checks which hosts are online without scanning ports.</td><td><pre><code>nmap -sn $target
</code></pre></td></tr><tr><td><strong>Scan with Scripts</strong></td><td>Runs specific NSE (Nmap Scripting Engine) scripts for vulnerability detection.</td><td><pre><code>nmap --script=$script_name $target
</code></pre></td></tr><tr><td><strong>Scan All Ports</strong></td><td>Scans all 65,535 TCP ports.</td><td><pre><code>nmap -p- $target
</code></pre></td></tr><tr><td><strong>Scan with Timing</strong></td><td>Adjusts the timing template for faster or slower scans.</td><td><pre><code>nmap -T4 $target
</code></pre></td></tr><tr><td><strong>Scan for Firewall</strong></td><td>Detects if a firewall is present and its rules.</td><td><pre><code>nmap -sA $target
</code></pre></td></tr><tr><td><strong>Scan for Top Ports</strong></td><td>Scans the top 1000 most common ports.</td><td><pre><code>nmap --top-ports 1000 $target
</code></pre></td></tr><tr><td><strong>Scan for IPv6</strong></td><td>Scans an IPv6 address.</td><td><pre><code>nmap -6 $target
</code></pre></td></tr><tr><td><strong>Traceroute</strong></td><td>Maps the network path to the target.</td><td><pre><code>nmap --traceroute $target
</code></pre></td></tr><tr><td><strong>Save Output</strong></td><td>Saves the scan results to a file.</td><td><pre><code>nmap -oN $output_file $target
</code></pre></td></tr><tr><td><strong>Scan Multiple Targets</strong></td><td>Scans multiple targets at once.</td><td><pre><code>nmap $target1 $target2
</code></pre></td></tr><tr><td><strong>Exclude Targets</strong></td><td>Excludes specific hosts from the scan.</td><td><pre><code>nmap $subnet --exclude $excluded_host
</code></pre></td></tr><tr><td><strong>Scan with Decoys</strong></td><td>Uses decoy IPs to obscure the source of the scan.</td><td><pre><code>nmap -D $decoy1,$decoy2,$target
</code></pre></td></tr><tr><td><strong>Idle Scan</strong></td><td>Performs a stealthy scan using a zombie host.</td><td><pre><code>nmap -sI $zombie_host $target
</code></pre></td></tr><tr><td><strong>Scan for Vulnerabilities</strong></td><td>Uses vulnerability detection NSE scripts.</td><td><pre><code>nmap --script=vuln $target
...
* **Types of Sockets**:
  * **Stream Sockets (SOCK\_STREAM)**: Used for reliable, connection-oriented communication (e.g., TCP).
  * **Datagram Sockets (SOCK\_DGRAM)**: Used for connectionless communication (e.g., UDP).
  * **Raw Sockets (SOCK\_RAW)**: Provide direct access to lower-level protocols, often used for custom protocol implementation or packet analysis.
* **Socket API**:\
...
<table data-header-hidden><thead><tr><th></th><th></th><th></th></tr></thead><tbody><tr><td><strong>Command</strong></td><td><strong>Description</strong></td><td><strong>Example</strong></td></tr><tr><td><code>ping</code></td><td>Checks if a host is reachable and alive.</td><td><pre><code>ping example.com
</code></pre></td></tr><tr><td></td><td><code>-c</code>: Specifies the number of packets to send.</td><td><pre><code>ping -c 4 example.com
</code></pre></td></tr><tr><td></td><td><code>-i</code>: Sets the interval between packets in seconds.</td><td><pre><code>ping -i 0.5 example.com
</code></pre></td></tr><tr><td><code>traceroute</code></td><td>Displays the route packets take to a network host.</td><td><pre><code>traceroute example.com
</code></pre></td></tr><tr><td></td><td><code>-n</code>: Prevents DNS resolution for faster results.</td><td><pre><code>traceroute -n example.com
</code></pre></td></tr><tr><td></td><td><code>-I</code>: Uses ICMP packets instead of UDP.</td><td><pre><code>traceroute -I example.com
</code></pre></td></tr><tr><td><code>netstat</code></td><td>Displays network connections, routing tables, and protocol statistics.</td><td><pre><code>netstat -a
</code></pre></td></tr><tr><td></td><td><code>-tuln</code>: Shows listening ports with protocol details.</td><td><pre><code>netstat -tuln
</code></pre></td></tr><tr><td></td><td><code>-i</code>: Displays network interface statistics.</td><td><pre><code>netstat -i
</code></pre></td></tr><tr><td><code>ss</code></td><td>Displays detailed socket statistics and network connections.</td><td><pre><code>ss -tuln
</code></pre></td></tr><tr><td></td><td><code>-s</code>: Shows summary statistics for sockets.</td><td><pre><code>ss -s
</code></pre></td></tr><tr><td></td><td><code>-p</code>: Displays processes using sockets.</td><td><pre><code>ss -p
</code></pre></td></tr><tr><td><code>arp</code></td><td>Displays or modifies the ARP cache.</td><td><pre><code>arp -a
</code></pre></td></tr><tr><td></td><td><code>-d</code>: Deletes a specific ARP entry.</td><td><pre><code>arp -d 192.168.1.1
</code></pre></td></tr><tr><td><code>ip</code></td><td>Configures and displays network interfaces and routing.</td><td><pre><code>ip addr show
</code></pre></td></tr><tr><td></td><td><code>link set</code>: Brings an interface up or down.</td><td><pre><code>ip link set eth0 up
</code></pre></td></tr><tr><td></td><td><code>route add</code>: Adds a new route to the routing table.</td><td><pre><code>ip route add 192.168.1.0/24 via 192.168.1.1
</code></pre></td></tr><tr><td><code>ip route</code></td><td>Displays or modifies the routing table.</td><td><pre><code>ip route show
</code></pre></td></tr><tr><td></td><td><code>add default</code>: Sets the default gateway.</td><td><pre><code>ip route add default via 192.168.1.1
</code></pre></td></tr><tr><td></td><td><code>del</code>: Removes a specific route.</td><td><pre><code>ip route del 192.168.1.0/24
</code></pre></td></tr><tr><td><code>ip neigh</code></td><td>Displays or modifies the neighbor cache (ARP table).</td><td><pre><code>ip neigh show
</code></pre></td></tr><tr><td></td><td><code>add</code>: Adds a static ARP entry.</td><td><pre><code>ip neigh add 192.168.1.2 lladdr 00:11:22:33:44:55 dev eth0
</code></pre></td></tr><tr><td></td><td><code>del</code>: Deletes a specific ARP entry.</td><td><pre><code>ip neigh del 192.168.1.2 dev eth0
</code></pre></td></tr><tr><td><code>ifconfig</code></td><td>Configures or displays network interfaces (deprecated in favor of <code>ip</code>).</td><td><pre><code>ifconfig
</code></pre></td></tr><tr><td></td><td><code>eth0 up</code>: Brings the interface up.</td><td><pre><code>ifconfig eth0 up
</code></pre></td></tr><tr><td></td><td><code>eth0</code>: Assigns an IP address and netmask to the interface.</td><td><pre><code>ifconfig eth0 192.168.1.100 netmask 255.255.255.0
</code></pre></td></tr><tr><td><code>host</code></td><td>Resolves domain names to IP addresses.</td><td><pre><code>host example.com
</code></pre></td></tr><tr><td></td><td><code>-t MX</code>: Queries for mail exchange records.</td><td><pre><code>host -t MX example.com
</code></pre></td></tr><tr><td></td><td><code>-a</code>: Displays all available DNS records.</td><td><pre><code>host -a example.com
</code></pre></td></tr><tr><td><code>dig</code></td><td>Queries DNS servers for domain information.</td><td><pre><code>dig example.com
</code></pre></td></tr><tr><td></td><td><code>+short</code>: Displays concise output.</td><td><pre><code>dig +short example.com
</code></pre></td></tr><tr><td></td><td><code>@8.8.8.8</code>: Uses a specific DNS server for the query.</td><td><pre><code>dig @8.8.8.8 example.com
</code></pre></td></tr><tr><td><code>whois</code></td><td>Retrieves domain registration information.</td><td><pre><code>whois example.com
</code></pre></td></tr><tr><td></td><td><code>-h</code>: Specifies a specific WHOIS server to query.</td><td><pre><code>whois -h whois.verisign-grs.com example.com
</code></pre></td></tr><tr><td><code>route</code></td><td>Displays or modifies the routing table.</td><td><pre><code>route -n
</code></pre></td></tr><tr><td></td><td><code>add default</code>: Adds a default gateway.</td><td><pre><code>route add default gw 192.168.1.1
</code></pre></td></tr><tr><td></td><td><code>del</code>: Removes a specific network route.</td><td><pre><code>route del -net 192.168.1.0/24
</code></pre></td></tr><tr><td><code>iptables</code></td><td>Configures firewall rules for packet filtering.</td><td><pre><code>iptables -L
</code></pre></td></tr><tr><td></td><td><code>-A</code>: Appends a rule to a chain.</td><td><pre><code>iptables -A INPUT -p tcp --dport 22 -j ACCEPT
</code></pre></td></tr><tr><td></td><td><code>-D</code>: Deletes a rule from a chain.</td><td><pre><code>iptables -D INPUT -p tcp --dport 22 -j ACCEPT
</code></pre></td></tr><tr><td><code>nft</code></td><td>Configures firewall rules using nftables (successor to iptables).</td><td><pre><code>nft list ruleset
</code></pre></td></tr><tr><td></td><td><code>add rule</code>: Adds a rule to a chain.</td><td><pre><code>nft add rule ip filter input tcp dport 22 accept
</code></pre></td></tr><tr><td></td><td><code>delete rule</code>: Removes a specific rule by handle.</td><td><pre><code>nft delete rule ip filter input handle 10
</code></pre></td></tr><tr><td><code>netcat</code> (<code>nc</code>)</td><td>A versatile tool for network debugging and testing.</td><td><pre><code>nc -zv example.com 80
</code></pre></td></tr><tr><td></td><td><code>-l</code>: Listens for incoming connections.</td><td><pre><code>nc -l 1234
</code></pre></td></tr><tr><td></td><td><code>echo</code>: Sends data to a remote host.</td><td><pre><code>echo "Hello" | nc example.com 1234
</code></pre></td></tr><tr><td><code>scp</code></td><td>Securely copies files between systems over SSH.</td><td><pre><code>scp file.txt user@example.com:/path/to/destination
</code></pre></td></tr><tr><td></td><td><code>-r</code>: Recursively copies directories.</td><td><pre><code>scp -r /local/dir user@example.com:/remote/dir
</code></pre></td></tr><tr><td></td><td>Copies a remote file to the local system.</td><td><pre><code>scp user@example.com:/remote/file.txt /local/dir
</code></pre></td></tr><tr><td><code>sftp</code></td><td>Securely transfers files using SSH.</td><td><pre><code>sftp user@example.com
</code></pre></td></tr><tr><td></td><td><code>put</code>: Uploads a file to the remote server.</td><td><pre><code>put localfile.txt
</code></pre></td></tr><tr><td></td><td><code>get</code>: Downloads a file from the remote server.</td><td><pre><code>get remotefile.txt
</code></pre></td></tr><tr><td><code>ssh</code></td><td>Securely connects to a remote system.</td><td><pre><code>ssh user@example.com
</code></pre></td></tr><tr><td></td><td><code>-p</code>: Specifies a custom port for the connection.</td><td><pre><code>ssh -p 2222 user@example.com
</code></pre></td></tr><tr><td></td><td><code>-i</code>: Uses a specific private key for authentication.</td><td><pre><code>ssh -i /path/to/key.pem user@example.com
</code></pre></td></tr></tbody></table>
...
<table data-header-hidden><thead><tr><th></th><th></th></tr></thead><tbody><tr><td><strong>Command</strong></td><td><strong>Description</strong></td></tr><tr><td><pre><code>show ip interface brief
</code></pre></td><td>Displays a summary of all interfaces, their IP addresses, and status.</td></tr><tr><td><pre><code>show running-config
</code></pre></td><td>Displays the current configuration in RAM.</td></tr><tr><td><pre><code>show startup-config
</code></pre></td><td>Displays the configuration stored in NVRAM (used on reboot).</td></tr><tr><td><pre><code>show version
</code></pre></td><td>Displays system information, including IOS version, uptime, and hardware.</td></tr><tr><td><pre><code>show ip route
</code></pre></td><td>Displays the routing table.</td></tr><tr><td><pre><code>show arp
</code></pre></td><td>Displays the ARP table.</td></tr><tr><td><pre><code>show interfaces
</code></pre></td><td>Displays detailed information about all interfaces.</td></tr><tr><td><pre><code>show protocols
</code></pre></td><td>Displays the status of configured Layer 3 protocols on all interfaces.</td></tr><tr><td><pre><code>ping $IP_Address
</code></pre></td><td>Sends ICMP echo requests to test connectivity to a specific IP address.</td></tr><tr><td><pre><code>traceroute $IP_Address
</code></pre></td><td>Traces the route packets take to reach a specific IP address.</td></tr><tr><td><pre><code>show cdp neighbors
</code></pre></td><td>Displays information about directly connected Cisco devices.</td></tr><tr><td><pre><code>show logging
</code></pre></td><td>Displays the router's log messages.</td></tr><tr><td><pre><code>show flash
</code></pre></td><td>Displays information about the router's flash memory.</td></tr><tr><td><pre><code>show ip nat translations
</code></pre></td><td>Displays the NAT translation table (if NAT is configured).</td></tr></tbody></table>
